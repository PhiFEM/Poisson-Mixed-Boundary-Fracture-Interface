...
class dirichlet_expr(df.UserExpression) : 
    def eval(self, value, x):
        if x[0] >= 0.5:
            value[0] = 1.0
        else:
...
class neumann_expr(df.UserExpression) : 
    def eval(self, value, x):
        if x[0] <= 0.5:
            value[0] = 1.0
        else:
...
for i in range(start,end,step): 
    ...
    boundary_penalty = sigma_p*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(Neumann) \
                     ...
                     + sigma_D*h**2*(df.inner(df.div(sigma(u)) ,df.div(sigma(v)) ))*(dx(Dirichlet)+dx(Interface))
    ...
    lv = df.inner(f,v)*dx  \
        + sigma_D*h**2*df.inner(f, - df.div(sigma(v)))*(dx(Dirichlet)+dx(Interface)) \
...
for i in range(start, end, step):
    ...
    # cut function to impose boundary conditions
    cut_neumann = neumann_expr(element = V_phi.ufl_element())
    cut_dirichlet = dirichlet_expr(element = V_phi.ufl_element())
    ...
    a = df.inner(sigma(u), epsilon(v))*df.dx + 1e10*cut_dirichlet*df.inner(u,v)*df.ds
    L = df.dot(f,v)*df.dx + cut_neumann*df.dot(g,v)*df.ds + 1e10*cut_dirichlet*df.inner(u_D,v)*df.ds
    start_assemble = time.time()
    A = df.assemble(a)
    B = df.assemble(L)
    end_assemble = time.time()
...
f = open('output_Phi_Fem_Mixed_Elasticity_interface_BC_P{name0}.txt'.format(name0=degV),'w')
