...
Time_assemble_phi_fem, Time_solve_phi_fem, Time_total_phi_fem, error_l2_phi_fem, error_h1_phi_fem = [], [], [], [], []
hh_phi_fem = []
...
for i in range(start,end,step): 
    ...
    W = mph.BlockFunctionSpace([V,Z,Q_N], restrict=[None,yp_res,yp_res])
    uyp = mph.BlockTrialFunction(W)
    (u, y, p_N) = mph.block_split(uyp)
    vzq = mph.BlockTestFunction(W)
    (v, z, q_N) = mph.block_split(vzq)
    
    gamma_div, gamma_u, gamma_p, sigma_p, sigma_N = 1.0, 1.0, 1.0, 0.01, 20.0
    ...
    boundary_penalty = sigma_N*df.avg(h)*df.inner(df.jump(sigma(u),n), df.jump(sigma(v),n))*dS(2) \
    ...
    auv = df.inner(sigma(u), epsilon(v))*dx + df.inner(u, v)*dx  \
        ...
        + gamma_div*df.inner(u,v)*dx(Neumann) \
        + boundary_penalty 

    auz = gamma_u*df.inner(sigma(u),z)*dx(Neumann) + gamma_div*df.inner(u, df.div(z))*dx(Neumann)
    auq_N = 0.0  
    
    ayv = df.inner(df.dot(y,n),v)*ds(Neumann) + gamma_u*df.inner(y,sigma(v))*dx(Neumann) + gamma_div*df.inner(df.div(y),v)*dx(Neumann)
    ...
    lv = df.inner(f,v)*dx +gamma_div * df.inner(f, v)*dx(Neumann)
    ...
    Time_assemble_phi_fem.append(end_assemble-start_assemble)
    UU = mph.BlockFunction(W)
    start_solve = time.time()
    mph.block_solve(A, UU.block_vector(), B)
    end_solve = time.time()
    Time_solve_phi_fem.append(end_solve-start_solve)
    Time_total_phi_fem.append(Time_assemble_phi_fem[-1] + Time_solve_phi_fem[-1])
    u_h = UU[0]
...
plt.tight_layout()
plt.savefig('relative_error_Phi_Fem_Neumannt_ElasticityP_{name0}.png'.format(name0=degV))
plt.show()

plt.figure()
plt.loglog(hh_phi_fem,Time_assemble_phi_fem, '-o',label=r'Assemble $\phi$-FEM')
plt.loglog(hh_phi_fem,Time_solve_phi_fem,'--o', label=r'Solve $\phi$-FEM')
...
plt.tight_layout()
plt.savefig('Time_precision_Phi_Fem_Neumann_Elasticity_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
plt.loglog(error_l2_phi_fem,Time_assemble_phi_fem, '-o',label=r'Assemble $\phi$-fem')
plt.loglog(error_l2_phi_fem,Time_solve_phi_fem,'--o', label=r'Solve $\phi$-fem')
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Time_error_Phi_Fem_Neumann_Elasticity_P_{name0}.png'.format(name0=degV))
plt.show()
plt.figure()
plt.loglog(error_l2_phi_fem,Time_total_phi_fem,'-o', label=r'$\phi$-fem')
...
plt.legend(loc='upper right')
plt.tight_layout()
plt.savefig('Total_time_error_Phi_Fem_Neumann_Elasticity_P_{name0}.png'.format(name0=degV))
plt.show()"""


#  Write the output file for latex
f = open('output_Phi_Fem_Neumann_Elasticity_P{name0}.txt'.format(name0=degV),'w')
...
output_latex(f, error_l2_phi_fem, Time_total_phi_fem)
f.write('relative H1 norm and time phi fem : \n')	
output_latex(f, error_h1_phi_fem, Time_total_phi_fem)
